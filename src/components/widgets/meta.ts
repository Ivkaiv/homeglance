...
export const ROOM_HUB_META: WidgetMeta = {
  ...
  paramSchema: [
    ...
    { key: 'tempDecimals', label: 'Знаков после запятой у температуры', kind: 'number',
      default: 0, step: 1, min: 0, max: 3,
      hint: 'По умолчанию «23°». Поставь 1 для «23.4°», 2 для «23.45°»',
    ...
    { key: 'showHumidity', label: 'Показывать влажность в шапке', kind: 'boolean', default: true,
      hint: 'Если выключено — значение скрыто, даже если датчик выбран', group: 'climate-header' },
    { key: 'humidityDecimals', label: 'Знаков после запятой у влажности', kind: 'number',
      default: 0, step: 1, min: 0, max: 3,
      hint: 'По умолчанию «56%». Поставь 1 для «56.4%»',
      group: 'climate-header' },
    ...
    { key: 'sensorEntities', label: 'Дополнительные датчики', kind: 'multi-entity',
      ...
      group: 'extra-sensors' },
    { key: 'sensorDecimals', label: 'Знаков после запятой', kind: 'entity-numbers',
      linkedKey: 'sensorEntities', step: 1, min: 0, max: 3,
      hint: 'Пусто = авто. Бинарные (двери, окна, движение) игнорируют это поле',
      group: 'extra-sensors' },
  ],
};
...
export const MULTI_SENSOR_META: WidgetMeta = {
  ...
  paramSchema: [
    ...
    },
    {
      ...
      min: 0,
      max: 3,
      hint: 'Пусто = авто (0 для температуры, 1 для влажности, и т.п.). 2 = «23.45°»',
    },
    {
      key: 'label',
      label: 'Подпись блока',
...
export const ZAYCEV_RADIO_META: WidgetMeta = {
  ...
  category: 'media',
  defaultSize: { w: 4, h: 2 },
  // Минимум 2 строки — на 1 строке виджет схлопывался в одинокий значок-
  // радио без заголовка и без play-кнопки, и был неотличим от случайной
  // лампочки. С h=2 уже помещается «иконка + название + play», как и
  // должно выглядеть радио на полке.
  minSize: { w: 3, h: 2 },
  paramSchema: [
    {
