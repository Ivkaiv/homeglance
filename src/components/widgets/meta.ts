...
export const ENERGY_META: WidgetMeta = {
  ...
  category: 'sensors',
  defaultSize: { w: 2, h: 2 },
  // 2×2 минимум — значение мощности + единица + название сенсора. На 2×1
  // (старая сетка 9×32) название обрезалось до «В.», а на новой сетке 4×80
  // в 2×1 = ~170×80px ещё помещается, но без места под подпись и доп. метрики.
  minSize: { w: 2, h: 2 },
  paramSchema: [
    {
      key: 'power',
...
export const MULTI_SENSOR_META: WidgetMeta = {
  ...
  category: 'sensors',
  defaultSize: { w: 3, h: 2 },
  // 2×2 минимум — два сенсорных chip в строке + иконка/значение/единица.
  // На 2×1 значение и подпись не уместятся одновременно.
  minSize: { w: 2, h: 2 },
  paramSchema: [
    {
      key: 'entities',
