...
export const ENERGY_META: WidgetMeta = {
  ...
  category: 'sensors',
  defaultSize: { w: 2, h: 2 },
  // 2×1 минимум — иначе на 1×1 единица измерения и значение не помещаются
  // в одну строку и слова «Мощность» не видно совсем.
  minSize: { w: 2, h: 1 },
  paramSchema: [
    {
      key: 'power',
...
export const LIGHT_COLOR_META: WidgetMeta = {
  ...
  category: 'lights',
  defaultSize: { w: 2, h: 1 },
  // 2×1 минимум — иконка + название + статус. На 1×1 текст не помещается
  // вообще, остаётся только иконка-toggle — без названия пользователь
  // не понимает что это за лампа.
  minSize: { w: 2, h: 1 },
  paramSchema: [
    {
      key: 'entity',
