...
export const ENERGY_META: WidgetMeta = {
  ...
  category: 'sensors',
  defaultSize: { w: 2, h: 2 },
  // 2×1 минимум — значение «407 W» + название сенсора на одной строке/двух
  // строках текста. На сетке 6×60 это ~120×60px, помещается «407 W» крупно
  // + строка подписи. Дополнительные метрики (kWh/стоимость) появляются
  // при росте до 2×2.
  minSize: { w: 2, h: 1 },
  paramSchema: [
    {
...
export const MULTI_SENSOR_META: WidgetMeta = {
  ...
  category: 'sensors',
  defaultSize: { w: 3, h: 2 },
  // 2×1 минимум — два чипа в строку. На 6×60 ≈ 120×60px, по 60px на чип
  // — иконка + значение в одну строку, без подписи. Подпись появляется
  // при росте до 2×2.
  minSize: { w: 2, h: 1 },
  paramSchema: [
    {
