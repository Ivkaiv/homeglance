...
interface SettingsViewProps {
  /**
   * Под HA Ingress sandbox iframe ломает window.location-навигацию,
   * поэтому settings рендерится inline на главной. Если prop передан,
   * кнопка «Назад» и кнопки disconnect не уходят на другой URL, а
   * сообщают родителю что нужно вернуться к Dashboard.
   */
  onBack?: () => void;
}
...
export default function SettingsView({ onBack }: SettingsViewProps = {}) {
  ...
  async function doDisconnect() {
    // forget() сбрасывает hasCredentials → ConnectionProvider → главная
    // на app/page.tsx покажет OnboardingPage inline. Достаточно вернуться
    // к dashboard-view (если рендеримся inline), всё остальное сделает state.
    await forget();
    onBack?.();
  }
