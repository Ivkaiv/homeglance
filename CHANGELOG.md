...
### Fixed
- **Музыка: «волна» и «избранное» падали с «Не удалось».** Причина: у
  Яндекс.Станции `current_media.uri` приходит как голый ID Яндекс.Музыки
  (без схемы `://`) — по такому адресу MA не строит радио и не добавляет в
  избранное. Теперь адрес проверяется: если он непригоден, берётся адрес
  из активной очереди плеера; если и там нет — кнопка честно сообщает
  «трек запущен не через Glance», а не показывает ложное «Не удалось».

## [0.1.0-alpha.80] — 2026-05-20

### Fixed
- **Музыка: обратная связь на действия.** Выбор трека сразу подсвечивает
  строку и крутит спиннер, всплывающая плашка подтверждает запуск; кнопки
  «Волна» и «Избранное» показывают состояние «отправляю…» и результат.
  Раньше виджет молча ждал событие сервера и казался «замороженным».
- **Музыка: обложки плейлистов Звука** не загружались — прокси картинок
  пропускал только `cdn-image.zvuk.com`, а плейлисты отдаются с `zvuk.com`.
  Добавлен домен в белый список + обработка плейсхолдера `{size}`.

### Changed
- **Музыка: открытый альбом/артист/плейлист** теперь показывается
  полноэкранным слоем с кнопкой «Назад», а не блоком над поиском.
- **Музыка: предупреждение** под списком выходов, если выбрана
  Яндекс.Станция (она рвёт очередь Music Assistant — для альбомов и
  плейлистов нужен другой плеер).

## [0.1.0-alpha.79] — 2026-05-20

### Added
- **Открытие альбома / артиста / плейлиста** на странице «Музыка». Тап по
  альбому или артисту в поиске больше не запускает всё сразу — открывается
  список треков с кнопкой «Играть всё» и возможностью включить конкретный
  трек. Кнопка «назад» возвращает к поиску.
- **«Волна»** — кнопка в плеере запускает бесконечный поток похожей музыки
  на основе текущего трека (радио-режим Music Assistant).
- **Избранное** — кнопка «В избранное» добавляет текущий трек в избранное
  источника (Звук).

## [0.1.0-alpha.78] — 2026-05-20

### Added
- **Очередь на странице «Музыка»:** кнопки перемешивания (shuffle) и повтора
  (off → all → one) в плеере, строка «Далее: …» со следующим треком.
- **Секция «Недавнее»** — недавно прослушанное, тап включает повторно.
- Обложки-миниатюры в результатах поиска и в «Недавнем».

## [0.1.0-alpha.77] — 2026-05-20

### Fixed
- **Не показывались обложки в плеере Music Assistant.** MA отдаёт ссылку на
  обложку по `http://` (imageproxy), а Glance под HA Ingress работает по
  `https` — браузер блокировал картинку как mixed-content. Добавлен
  серверный прокси обложек `/api/glance/ma-image` (тянет картинку
  server-side, отдаёт same-origin, SSRF-защита по списку хостов). Обложки
  теперь грузятся и в плеере, и в виджете.

### Added
- Обложки в результатах поиска на странице «Музыка» (миниатюра вместо
  иконки типа).

## [0.1.0-alpha.76] — 2026-05-20

### Fixed
- **Страница «Музыка» открывалась пустой.** При создании страницы её
  конфигурация (`music`) не сохранялась — `addPage` переносил только поле
  `weather`, — и Dashboard не находил конфиг, показывая пустую сетку.
  Теперь `music` сохраняется, а страница рендерится по `kind='music'`
  даже без конфига (уже созданные пустые страницы тоже починятся).

## [0.1.0-alpha.75] — 2026-05-20

### Added
- **Отдельная страница «Музыка».** Полноэкранная страница плеера Music
  Assistant — отдельная вкладка в dock-баре, а не виджет на сетке. Создаётся
  через «Страницы» → новая страница → тип «🎵 Музыка». Содержит плеер
  (обложка, прогресс, transport, громкость), выбор устройства вывода и
  **поиск музыки** по медиатеке источников: тап по результату включает
  трек / альбом / плейлист на выбранном устройстве.
- Команда `music/search` в WS-клиенте Music Assistant; новый тип страницы
  `music` (по образцу страницы «Погода»).

## [0.1.0-alpha.74] — 2026-05-20

### Fixed
- **Плеер Music Assistant: трек стартовал с ~20 сек на полоске прогресса.**
  Позиция экстраполировалась от серверного `elapsed_time_last_updated`, но
  часы MA-сервера и браузера расходятся — отсюда постоянный сдвиг. Теперь
  прогресс привязан к локальному времени получения данных, а клиент чаще
  перечитывает состояние (по событию `queue_time_updated`), поэтому
  `elapsed_time` остаётся свежим.

## [0.1.0-alpha.73] — 2026-05-20

### Added
- **Виджет «Музыка (Music Assistant)» — Этап 1.** Новый виджет музыкального
  плеера, который общается с отдельным сервером Music Assistant. Карточка
  показывает обложку и текущий трек; попап даёт полное управление: прогресс
  с перемоткой, transport-кнопки, громкость и **выбор выхода** — список всех
  устройств Music Assistant (приставка, ТВ, колонки), тапом переключаешь,
  на каком играть.
- Слой связи Glance ↔ Music Assistant: `src/lib/music/` (типы, конфиг,
  WS-клиент `MAClient`, `MusicProvider` с хуками). Подключение ленивое —
  WS открывается только когда на экране есть музыкальный виджет.
- `server.js`: WS-прокси `/api/glance/ma-ws` — аддон сам авторизуется в
  Music Assistant, токен в браузер не уходит. URL и токен задаются в
  настройках аддона (`music_assistant_url` / `music_assistant_token`).

## [0.1.0-alpha.72] — 2026-05-19

### Added
- **Радио Zaycev FM встроено в попап медиа-плеера.** В настройках виджета
  «Медиа-плеер» появилась галочка «Радио Zaycev FM в попапе» и выбор качества
  стрима (48/128/256 kbps). Когда галочка включена, во всплывающем окне плеера
  снизу появляется сворачиваемая секция «Радио · Zaycev FM»: тап разворачивает
  сетку из 17 жанровых каналов, тап по каналу запускает стрим на том же
  плеере. Активный канал подсвечивается акцентным цветом обложки, его название
  видно в свёрнутой строке. При закрытии попапа секция снова сворачивается.

### Removed
- **Отдельный виджет «Zaycev FM» удалён.** Его функции переехали в попап
  медиа-плеера (см. выше). Уже стоящий на дашборде виджет `zaycev_radio`
  после обновления покажет мягкую заглушку «Unknown widget» — его нужно
  удалить вручную и включить галочку «Радио» в виджете плеера.

### Changed
- Данные каналов Zaycev FM вынесены в общий модуль `src/lib/zaycev.ts` —
  чистый data-модуль без React, импортируется попапом плеера.
...
### Changed
- **Минимум для виджета Zaycev FM поднят до 3×2.** На 2×1 виджет
  схлопывался в одинокий значок-радио в тонкой полоске — без названия,
  без play-кнопки, неотличим от какой-нибудь лампы. С `minSize 3×2`
  всегда помещается «иконка + название + play», как и должна
  выглядеть радио-карточка.

## [0.1.0-alpha.69] — 2026-05-18

### Changed
- **Виджет Zaycev FM полностью переработан под формат плеера.** Прошлая
  версия (alpha.68) висела на дашборде огромной сеткой плашек со всеми
  18 каналами — занимала много места и выглядела чужеродно рядом с
  обычным `MediaPlayerWidget`. Теперь карточка компактная, в духе
  обычного плеера, с теми же тремя tier'ами (tiny / small / horizontal /
  vertical):
  - Иконка-радио в тёплой янтарной плашке (вместо обложки трека);
    когда канал играет — вместо иконки эмодзи канала.
  - Заголовок «Pop / Rock / …», в подзаголовке бейдж «📻 РАДИО» —
    сразу отличает от обычного плеера.
  - Transport-кнопки play/pause/stop прямо в карточке.
  - Тап по обложке/площади открывает попап `ZaycevRadioSheet`, в котором
    лежат сетка всех каналов (4 колонки), большие play/stop и слайдер
    громкости с mute-кнопкой. Всё в одном месте, без перегрузки дашборда.

### Added
- **`ZaycevRadioSheet`** — новый модальный попап с полным управлением:
  сетка каналов, transport, громкость. Открывается тапом по виджету.
...
### Added
- **Кастомный цвет свечения для виджета «Переключатель».** Раньше у
  switch'а цвет подсветки был захардкожен зелёным, у `LightToggleWidget`
  поле было давно. Теперь у обоих в настройках есть поле «Цвет свечения
  когда включён»: для вентилятора удобно поставить голубой, для
  обогревателя — оранжевый и т.п.
- **Цвет свечения каждой кнопки в RoomHubWidget.** Через новый тип
  параметра `entity-colors` (связка с multi-entity через `linkedKey`):
  напротив каждой лампы / переключателя в настройках хаба есть свой
  color-picker. Пусто — используется дефолт (янтарный для ламп, accent
  для переключателей). Можно сбросить через крестик.

## [0.1.0-alpha.66] — 2026-05-18

### Changed
- **Кнопка цвета у цветной лампы стала заметной и понятной.** Раньше это
  была маленькая 16-пиксельная точка с тонким ring'ом — на тёмном фоне
  почти не читалась, и совсем не было понятно «это кнопка». Теперь:
  6×6 круг с белой иконкой `Palette` внутри, двойной ring для контраста,
  тень. Когда лампа включена — кнопка светится её актуальным цветом;
  когда выключена — радужный conic-gradient, чтобы было очевидно «здесь
  можно выбрать цвет».

### Added
- **Поле «Иконка» в настройках виджета цветной лампы.** У `LightToggleWidget`
  оно было, у `LightColorWidget` — нет: пользователь не мог поменять `💡`
  на «led-strip», «wall-sconce-flat» и т.п. Теперь есть, дефолт `lightbulb`,
  принимает mdi-имя или эмодзи.

## [0.1.0-alpha.65] — 2026-05-18

### Changed
- **Цветная лампа в compact-режиме визуально унифицирована с обычной.**
  Раньше `LightColorWidget` в маленькой клетке выглядел чужеродно на
  полке с обычными лампами: маленькая Power-иконка в кружочке слева,
  «Лента / выключена» справа — совсем не как «Трек» и «Стол» рядом
  (большая иконка по центру, подпись снизу). Теперь компактная цветная
  лампа использует тот же `EntityToggleShell`, что и обычная: большая
  иконка-лампочка по центру, подпись снизу, glow в актуальном цвете лампы
  когда «вкл». Тап по площади → toggle (как у обычной). В правом верхнем
  углу — маленькая цветная точка-кнопка: открывает sheet с яркостью,
  цветом и пресетами. Цвет точки = актуальный цвет лампы.

### Added
- **`EntityToggleShell` получил slot `cornerButton`** — опциональная
  кнопка в правом верхнем углу. Пригодится для других виджетов, где
  основное действие = toggle, а дополнительное = открыть sheet
  (например, климат → расширенные режимы, переключатель → расписание).

## [0.1.0-alpha.64] — 2026-05-18

### Added
- **Настраиваемая точность отображения для всех сенсорных виджетов.**
  Раньше температура комнаты в шапке RoomHub всегда показывалась
  `Math.round`'ом до целого — даже если датчик отдавал «23.4°». Теперь
  можно задать знаки после запятой:
  - **RoomHubWidget** — отдельные поля «Знаков после запятой у температуры»
    и «у влажности» в шапке, а в блоке доп.датчиков — entity-numbers-список
    «Знаков после запятой» для каждого чипа.
  - **MultiSensorWidget** — entity-numbers-список «Знаков после запятой»:
    можно для каждого сенсора в блоке задать своё (термометр 1, давление 0).
  - **SensorChip** теперь принимает override `decimals`, иначе как раньше
    берёт из preset (0 для температуры, 1 для влажности, и т.д.).
- **SensorValueWidget уже имел поле `decimals` в настройках** — без
  изменений, просто упоминается для полноты.
